...
def get_indices_withlt_bundle(ub_pairs, user_nums):
    ##对每个用户交互过的bundle来划分训练集
    # init train_idx, val_idx and test_idx
    train_idx = []
    val_idx = []
    test_idx = []
    ...
    for user in range(user_nums):
        # get all pairs index of this user
        user_pair = [i for i, pair in enumerate(ub_pairs) if pair[0] == user]

        # shuffle the index
        np.random.shuffle(user_pair)
        
        # get the number of pairs
        num_pairs = len(user_pair)
        num_val = int(0.1 * num_pairs)
        num_train = num_pairs - 2 * num_val
        
        # add the index to train_idx, val_idx and test_idx
        train_idx.extend(user_pair[:num_train])
        val_idx.extend(user_pair[num_train:num_train+num_val])
        test_idx.extend(user_pair[num_train+num_val:])

    return train_idx, val_idx, test_idx, test_idx

def get_indices_bundle(dataset, api_freq, threshold=4):
    data_idx = list(range(len(dataset)))
    shuffle(data_idx)
    split_num = int(len(dataset) / 10)
    val_idx = data_idx[:split_num]
    ...
    for i in test_idx:
        for api in np.where(dataset[i] == 1)[0]:
            flag = False
