...
# rootPath += "/BundleRecommendation"
import torch
from tools.utils import *
# from utils import tokenize, draw_counter_pic
...
class MashupDataset(Dataset):
    def __init__(self, all_api=False, api_threshold=1647):
        ...
        # generate bundle indices through combining the APIs that exist in the same mashup.
        # self.bundle_indices = []
        # for user_index in range(self.num_mashup):
        #     user_items = self.mashup_api_matrix[user_index]

        #     # get the indices of non-zero items
        #     item_indices = np.where(user_items)[0]

        #     # get the indices of both items combination
        #     # itertools.combinations(item_indices, 3)
        #     if len(item_indices) > 1:
        #         # bundle_indice = list(itertools.combinations(item_indices, 3))
        #         bundle_indice = list(itertools.combinations(item_indices, len(item_indices)))

        #         # 如果 bundle_indice 已经存在于 bundle_indices 中，则不再添加
        #         # if any(
        #         #         set(bundle_indice) == set(bundle) for bundle in self.bundle_indices
        #         #     ):
        #         #     # if the bundle_indice already exists in the bundle_indices, then don't add it again
        #         #     # remove the duplicate indices, and then add the new bundle_indice of the size - 1
        #         #     print("bundle_indice: {0} already exists".format(bundle_indice))
        #         #     self.bundle_indices.remove(bundle_indice)
        #         #     bundle_indice = list(itertools.combinations(item_indices, len(item_indices) - 1))

        #         self.bundle_indices.extend(bundle_indice)

        # self.bundle_indices = [tuple(itertools.combinations(np.where(user_items)[0], len(np.where(user_items)[0]))) for user_items in self.mashup_api_matrix if len(np.where(user_items)[0]) > 1]
        ...
        # self.train_idx, self.val_idx, self.test_idx, self.sample_lt = get_indices_withlt_bundle(self.ub_pairs, self.mashup_api_matrix, self.api_freq, self.num_mashup, 4)
        
        # mashup index: train / val / test / sample
        ...
        # [ub_graph, ui_graph, bi_graph]
        # init graph
        self.ub_graph = sp.coo_matrix((self.train_ub_matrix), shape=(self.num_mashup, self.num_bundle)).tocsr()
        self.ui_graph = sp.coo_matrix((self.mashup_api_matrix), shape=(self.num_mashup, self.num_api)).tocsr()
        self.bi_graph = sp.coo_matrix((self.bundle_item_matrix), shape=(self.num_bundle, self.num_api)).tocsr()
        self.neg_sample = 1
    ...
    def get_dataset_matrix(self, inds):
        ...
        for user in inds:
            all_bundles = self.user_bundle_matrix[user]
        ...
        data_ub_pairs = [self.ub_pairs[idx] for idx in inds]
        data_ub_matrix = np.zeros((self.num_mashup, self.num_bundle), dtype=int)

        # set the train_ub_matrix
        for user, bundle in data_ub_pairs:
            data_ub_matrix[user, bundle] = 1 
    ...
    def __getitem__(self, index):
        if torch.is_tensor(index):
            index = index.tolist()
        # user_b = index
        # all_bundles = self.user_bundle_matrix[index].nonzero()[0]
        user_b, pos_bundle = self.ub_pairs[index]
        all_bundles = list([pos_bundle])
        ub_matrix = self.train_ub_matrix[user_b]
        ub_matrix_eva = self.train_ub_matrix[user_b]
        ui_matrix = self.used_api_mlb.transform([self.used_api[user_b]])

        if index in self.train_idx:
            while True:
                i = np.random.randint(self.num_bundle)
                if self.ub_graph[user_b, i] == 0 and not i in all_bundles:
                    all_bundles.append(i)
                    if len(all_bundles) >= self.neg_sample + 1:
                        break
        
        elif index in self.val_idx:
            ub_matrix_eva = self.val_ub_matrix[user_b]
        
        elif index in self.test_idx:
            ub_matrix_eva = self.test_ub_matrix[user_b]

        return torch.tensor(user_b).long(), torch.tensor(all_bundles).long(),  \
            torch.tensor(ub_matrix, dtype=torch.float),  \
            torch.tensor(ub_matrix_eva, dtype=torch.float), \
            torch.tensor(ui_matrix, dtype=torch.long).squeeze(), \
...
class TextDataset:
    def __init__(self, mul=1, is_random=False):
        ...
        self.max_doc_len = 50
        # self.vectors = Vectors(name=rootPath + '/tools/glove.6B.300d.txt', cache=cache)
        # self.vectors = Vectors(name=rootPath + '/tools/glove/glove.6B.100d.txt', cache=cache)
        self.vectors = Vectors(name=rootPath + '/tools/glove/glove.6B.200d.txt', cache=cache)
        # self.vectors = Vectors(name=rootPath + '/tools/glove/glove.6B.300d.txt', cache=cache)
        ...
        # self.field.build_vocab(self.mashup_ds.mashup_des, self.api_ds.description, vectors=self.vectors, min_freq=1,
        #                        max_size=self.max_vocab_size)
    ...
    def word2id(self, is_random):
        ...
        for i, des in enumerate(self.mashup_ds.description):
            ...
            # tokens = des
            # tokens_ori = des
            # tokens_ori = ' '.join(map(str, tokens))
            ...
            if len(tokens) < self.max_doc_len:
                ...
                # tokens_ori = tokens_ori + "1" * (self.max_doc_len - len(tokens))
            else:
                tokens = tokens[:self.max_doc_len]
                # tokens_ori = tokens_ori[:self.max_doc_len]
            self.mashup_ds.description[i] = tokens
            # self.mashup_ds.des[i] = tokens_ori
    ...
    def tag2feature(self):
        for i, category in enumerate(self.mashup_ds.category):
            ...
            # tokens = [x for x in tokenize(' '.join(category))]
            if not tokens:
                tokens = [0]
            if len(tokens) < 10:
