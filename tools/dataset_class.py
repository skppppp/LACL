...
class MashupDataset(Dataset):
    def __init__(self, all_api=False):
        ...
        # with open(rootPath + '/data/mashup_des_all.json', encoding='utf8') as f:
        #     # .read()获得所有内容； .split('\n')按'\n'分割成列表
        #     self.mashup_des = f.read().split('\n')
        ...
        if all_api:
            ...
            self.API_THRESHOLD = len(api_list) * 1
            # self.API_THRESHOLD = len(api_list_all)
        ...
        self.api_api_matrix = np.zeros((self.num_api, self.num_api), dtype='float32')

        mashup_api_link = 0
        for i in range(len(self.used_api)):
            self.mashup_api.setdefault(i, {})
            api_list = self.used_api_mlb.transform([self.used_api[i]])
            # self.mashup_api_matrix[i] = api_list[0].tolist()
            # self.mashup_api_matrix[i] = np.array(api_list[0], dtype='float32')
            api_ids = np.where(api_list[0] == 1)
            # if len(api_ids[0]) > 4:
            #     print("{0}==>{1}".format(i, self.name[i]))
            for j in api_ids[0]:
            ...
            for i in api_ids[0]:
                for j in api_ids[0]:
                    self.api_api_matrix[i][j] += 1
                    self.api_api_matrix[j][i] += 1
...
class TextDataset:
    def __init__(self):
        ...
        if not os.path.exists(cache):
            os.mkdir(cache)
        self.mashup_ds = MashupDataset(all_api=False)
        self.api_ds = ApiDataset(all_api=False)
...
class TextDataset_Transform:
    ...
    def word2id(self):
        for i, des in enumerate(self.mashup_ds.description):
            ...
            if len(tokens) < self.max_doc_len:
                tokens.extend([1] * (self.max_doc_len - len(tokens)))
                tokens_ori = tokens_ori + tokens_ori[1] * (self.max_doc_len - len(tokens))
            else:
                tokens = tokens[:self.max_doc_len]
    ...
    def tag2feature(self):
        for i, category in enumerate(self.mashup_ds.category):
            tokens = [self.field.vocab.stoi[x] for x in tokenize(' '.join(category))]
            # tokens = [self.tokenizer(x, return_tensors='pt') for x in tokenize(' '.join(category))]
            if not tokens:
                tokens = [0]
            if len(tokens) < 10:
                tokens.extend([1] * (10 - len(tokens)))
    ...
    def word2id(self):
        for i, des in enumerate(self.mashup_ds.description):
            ...
            if len(tokens) < self.max_doc_len:
                ...
                # tokens_ori = tokens_ori[:self.max_doc_len]
            self.mashup_ds.description[i] = tokens
            self.mashup_ds.des[i] = ' '.join([str(x) for x in tokens])
        ...
        for i, des in enumerate(self.api_ds.description):
            ...
            if len(tokens) < self.max_doc_len:
                ...
                # tokens_ori = tokens_ori[:self.max_doc_len]
            self.api_ds.description[i] = tokens
            self.api_ds.des[i] = ' '.join([str(x) for x in tokens])
...
class FCDataset(Dataset):
    def __init__(self, sample_indices, ds, is_training=True):
